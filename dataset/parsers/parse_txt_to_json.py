...
def txt_to_json(eng_files: List[str], ukr_files: List[str], path_to_save: str, max_len: int):
...
def parse(en_file_name: str, ukr_files: List[str], max_len: int) -> ParsedResult:
    ...
    ukr_file_part = en_file_name.split('/')[-1].replace('.en', '')
    ukr_file_name = [f for f in ukr_files if ukr_file_part in f].pop()
    print(f'Ukrainian file {ukr_file_name}')
    en_lines = get_lines_from_txt(en_file_name)
    ukr_lines = get_lines_from_txt(ukr_file_name)
